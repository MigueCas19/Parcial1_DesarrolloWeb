...
template <typename TipoLlave>
void ArbolB<TipoLlave>::imprimirEntre(TipoLlave n1, TipoLlave n2)
{
	NodoB* nodo1;
	NodoB* nodo2;
	NodoB* temp;
	...
	nodo1 = this->encontrarHoja(n1);
	nodo2=this->encontrarHoja(n2);
	
	this->imprimirArbol(nodo1);
	this->imprimirArbol(nodo2);
	int num;

	for(int i = 0 ; i< nodo1->numLlaves;i++)
	{
		num = i;
		if(nodo1->llaves[i]>=n1)
		{
			break;
		}
	}
	for(int i = num;i< nodo1->numLlaves;i++)
	{
		std::cout << "- - - - - - -" << std::endl;
		tupla =nodo1->tuplas[i];
		std::cout << nodo1->llaves[i] << "||"<<tupla->getApellido()<< "||" << tupla->getNombre() << endl;
	...
	{
		std::cout << "- - - - - - -" << std::endl;
		for (int i = 0; i < temp->numLlaves; i++)
		{
			tupla = temp->tuplas[i];
			std::cout << temp->llaves[i] << "||"<<tupla->getApellido()<< "||" << tupla->getNombre() << endl;
	...
	std::cout << "Ya lo imprimio" << std::endl;
